...
const Home = () => {
    ...
    const userContact = async () => {
        try {
            ...
            // Check response status for errors before parsing
            if (!res.ok) {
                const error = new Error(`Error fetching data: ${res.statusText}`);
                throw error;
            }

            // Handle HTML content gracefully if necessary (assuming API can return HTML in error cases)
            const contentType = res.headers.get('Content-Type');
            if (contentType?.includes('text/html')) {
                console.warn('API returned HTML content. Consider handling it appropriately.');
                // Handle HTML content here (e.g., display an error message)
                return; // Exit the function early if HTML is encountered
            }
